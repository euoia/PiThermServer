...
function insertTemp(data) {
	console.log("Inserting temperature " + data.temperature_record[0].celsius + " at " + data.temperature_record[0].unix_time);
...
var server = http.createServer(
	...
	function (request, response) {
		...
		if (pathfile === '/temperature_query.json') {
			...
			if (query.num_obs) {
				num_obs = parseInt(query.num_obs, 2);
			} else {
...
// Start temperature logging (every 30 seconds).
var msecs = 30 * 1000; // log interval duration in milliseconds
logTemp(msecs);
